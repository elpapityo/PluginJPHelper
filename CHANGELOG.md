...
- Dalamud起動直後、「すべてのプラグイン」側で一部の説明文が英語のまま残ることがある問題を修正。
- Plugin InstallerのRemote Manifest読み込み完了前にPJHの翻訳処理が走った場合でも、起動後に段階的に再確認して翻訳を再適用するよう改善。
- PJHを一度OFF→ONしなくても、起動後数秒待てば説明文が自動的に日本語化されるよう修正。
